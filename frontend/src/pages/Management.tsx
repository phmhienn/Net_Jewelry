...
function ManagerCreate({ section, onSaved }: { section: ManageFormSection; onSaved: () => void }) {
  ...
  const title = section === "catalog" ? "Thêm danh mục / thương hiệu" : section === "coupons" ? "Tạo mã giảm giá" : section === "gold" ? "Thêm giá vàng" : section === "banner" ? "Thêm banner" : "Thêm nội dung";
...
function ManagerForm({ section, row, path, onSaved }: { section?: ManageFormSection; row?: Row; path?: string; onSaved: () => void }) {
  ...
  return <form className="management-form" onSubmit={(event) => void submit(event)}>
    ...
    {(effectiveSection === "category" || effectiveSection === "brand") && <><Input label="Tên" name="name" defaultValue={inputValue(row ?? {}, "name")} required />{effectiveSection === "category" && <Input label="ID danh mục cha" name="parentId" defaultValue={inputValue(row ?? {}, "parentId")} />}{effectiveSection === "brand" && <Input label="Logo" name="logo" defaultValue={inputValue(row ?? {}, "logo")} />}<label>Mô tả<textarea name="description" defaultValue={inputValue(row ?? {}, "description")} /></label><StatusSelect defaultValue={inputValue(row ?? {}, "status") || "HOAT_DONG"} /></>}
...
function PaymentsAdmin() { return <SimplePage title="Thanh toán & giao dịch" description="Theo dõi thanh toán COD và giao dịch của đơn hàng." path="/admin/payments" columns={[["id", "ID"], ["orderId", "Đơn"], ["amount", "Số tiền"], ["method", "Phương thức"], ["status", "Trạng thái"]]} />; }
function CatalogAdmin() {
  ...
  return <><PageHeader title="Danh mục & thương hiệu" description="Quản lý danh mục và thương hiệu hiển thị trên website." action={<ManagerCreate section="catalog" onSaved={refresh} />} />{error ? <ErrorState message={error} retry={refresh} /> : <div className="management-stack"><section><h2 className="management-section-title">Danh mục</h2><DataTable rows={categories} columns={[["id", "ID"], ["name", "Tên"], ["parentId", "Danh mục cha"], ["description", "Mô tả"], ["status", "Trạng thái"]]} actions={(row) => <ManagerRowAction path="/categories" section="category" row={row} onSaved={refresh} />} /></section><section><h2 className="management-section-title">Thương hiệu</h2><DataTable rows={brands} columns={[["id", "ID"], ["name", "Tên"], ["logo", "Logo"], ["description", "Mô tả"], ["status", "Trạng thái"]]} actions={(row) => <ManagerRowAction path="/brands" section="brand" row={row} onSaved={refresh} />} /></section></div>}</>;
}
...
function ContentAdmin() {
  ...
  return <><PageHeader title="Banner & nội dung" description="Quản lý banner và các trang thông tin public." action={<ManagerCreate section="content" onSaved={refresh} />} />{error ? <ErrorState message={error} retry={refresh} /> : <div className="management-stack"><section><h2 className="management-section-title">Banner</h2><DataTable rows={banners} columns={[["title", "Tiêu đề"], ["image", "Ảnh"], ["link", "Liên kết"], ["sortOrder", "Thứ tự"], ["status", "Trạng thái"]]} actions={(row) => <ManagerRowAction path="/admin/banners" section="banner" row={row} onSaved={refresh} />} /></section><section><h2 className="management-section-title">Trang nội dung</h2><DataTable rows={contents} columns={[["title", "Tiêu đề"], ["type", "Loại"], ["slug", "Slug"], ["status", "Trạng thái"]]} actions={(row) => <ManagerRowAction path="/admin/contents" section="page" row={row} onSaved={refresh} />} /></section></div>}</>;
}
