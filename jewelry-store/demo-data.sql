...
-- Các bản ghi do file tạo mang tiền tố DEMO hoặc demo_ để dễ nhận biết.
-- ============================================================
...
-- Vai trò và ba tài khoản kiểm tra. Mật khẩu của cả ba là: password
...
SELECT
  'Khách hàng kiểm tra', 'demo.customer@netjewelry.local', '0900000001',
  'demo_customer', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy',
  id, 'HOAT_DONG'
FROM vai_tro WHERE ten_vai_tro = 'KHACH_HANG'
ON DUPLICATE KEY UPDATE
  ho_ten = VALUES(ho_ten), so_dien_thoai = VALUES(so_dien_thoai),
  vai_tro_id = VALUES(vai_tro_id), trang_thai = 'HOAT_DONG';

INSERT INTO tai_khoan
  (ho_ten, email, so_dien_thoai, ten_dang_nhap, mat_khau, vai_tro_id, trang_thai)
SELECT
  'Nhân viên kiểm tra', 'demo.staff@netjewelry.local', '0900000002',
  'demo_staff', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy',
  id, 'HOAT_DONG'
...
INSERT INTO tai_khoan
  (ho_ten, email, so_dien_thoai, ten_dang_nhap, mat_khau, vai_tro_id, trang_thai)
SELECT
  'Quản lý kiểm tra', 'demo.manager@netjewelry.local', '0900000003',
  'demo_manager', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy',
  id, 'HOAT_DONG'
...
SET @demo_customer_id := (SELECT id FROM tai_khoan WHERE ten_dang_nhap = 'demo_customer');
SET @demo_staff_id := (SELECT id FROM tai_khoan WHERE ten_dang_nhap = 'demo_staff');
SET @demo_manager_id := (SELECT id FROM tai_khoan WHERE ten_dang_nhap = 'demo_manager');

INSERT INTO dia_chi
  (tai_khoan_id, ho_ten_nguoi_nhan, so_dien_thoai, tinh_thanh, quan_huyen, phuong_xa, dia_chi_chi_tiet, mac_dinh)
SELECT @demo_customer_id, 'Khách hàng kiểm tra', '0900000001', 'TP. Hồ Chí Minh', 'Quận 1', 'Phường Bến Nghé', '12 Lê Lợi', TRUE
WHERE NOT EXISTS (
  SELECT 1 FROM dia_chi WHERE tai_khoan_id = @demo_customer_id AND dia_chi_chi_tiet = '12 Lê Lợi'
);
...
VALUES
  ('DEMO-RING-001', 'Nhẫn vàng NÉT Mảnh', @cat_ring, @brand_net, 'Nhẫn vàng 18K dáng mảnh, hoàn thiện đánh bóng nhẹ.', 3450000.00, 3150000.00, '12', 'Vàng', 'Vàng 18K', 0.780, NULL, 'DANG_BAN'),
  ('DEMO-RING-002', 'Nhẫn đá Moonstone', @cat_ring, @brand_angia, 'Nhẫn vàng 18K đính moonstone xanh dịu.', 5200000.00, NULL, '14', 'Vàng', 'Vàng 18K', 1.080, 'Moonstone', 'DANG_BAN'),
  ('DEMO-NECK-001', 'Dây chuyền NÉT Tròn', @cat_necklace, @brand_net, 'Dây chuyền vàng 18K với mặt tròn tối giản.', 4890000.00, 4590000.00, '45 cm', 'Vàng', 'Vàng 18K', 1.250, NULL, 'DANG_BAN'),
  ('DEMO-NECK-002', 'Dây chuyền Bạc Aurora', @cat_necklace, @brand_aurora, 'Dây bạc 925 nhẹ, phù hợp sử dụng mỗi ngày.', 1250000.00, NULL, '42 cm', 'Bạc', 'Bạc 925', 2.100, NULL, 'DANG_BAN'),
  ('DEMO-BRACELET-001', 'Lắc tay vàng Mây', @cat_bracelet, @brand_angia, 'Lắc tay vàng 18K liên kết mềm mại.', 6750000.00, NULL, '16 cm', 'Vàng', 'Vàng 18K', 1.650, NULL, 'DANG_BAN'),
  ('DEMO-EARRING-001', 'Bông tai Ngọc trai NÉT', @cat_earring, @brand_net, 'Bông tai vàng 18K đính ngọc trai nước ngọt.', 2850000.00, 2650000.00, NULL, 'Vàng', 'Vàng 18K', 0.620, 'Ngọc trai', 'DANG_BAN')
ON DUPLICATE KEY UPDATE
...
SET @p_ring_1 := (SELECT id FROM san_pham WHERE ma_sku = 'DEMO-RING-001');
SET @p_ring_2 := (SELECT id FROM san_pham WHERE ma_sku = 'DEMO-RING-002');
SET @p_neck_1 := (SELECT id FROM san_pham WHERE ma_sku = 'DEMO-NECK-001');
SET @p_neck_2 := (SELECT id FROM san_pham WHERE ma_sku = 'DEMO-NECK-002');
SET @p_bracelet := (SELECT id FROM san_pham WHERE ma_sku = 'DEMO-BRACELET-001');
SET @p_earring := (SELECT id FROM san_pham WHERE ma_sku = 'DEMO-EARRING-001');
...
VALUES
  (@p_ring_1, 'DEMO-RING-001-12', '12', 'Vàng', 3450000.00, 3150000.00, 'DANG_BAN'),
  (@p_ring_2, 'DEMO-RING-002-14', '14', 'Vàng', 5200000.00, NULL, 'DANG_BAN'),
  (@p_neck_1, 'DEMO-NECK-001-45', '45 cm', 'Vàng', 4890000.00, 4590000.00, 'DANG_BAN'),
  (@p_neck_2, 'DEMO-NECK-002-42', '42 cm', 'Bạc', 1250000.00, NULL, 'DANG_BAN'),
  (@p_bracelet, 'DEMO-BRACELET-001-16', '16 cm', 'Vàng', 6750000.00, NULL, 'DANG_BAN'),
  (@p_earring, 'DEMO-EARRING-001', NULL, 'Vàng', 2850000.00, 2650000.00, 'DANG_BAN')
ON DUPLICATE KEY UPDATE
  gia = VALUES(gia), gia_khuyen_mai = VALUES(gia_khuyen_mai), trang_thai = 'DANG_BAN';

SET @v_ring_1 := (SELECT id FROM bien_the_san_pham WHERE sku = 'DEMO-RING-001-12');
SET @v_ring_2 := (SELECT id FROM bien_the_san_pham WHERE sku = 'DEMO-RING-002-14');
SET @v_neck_1 := (SELECT id FROM bien_the_san_pham WHERE sku = 'DEMO-NECK-001-45');
SET @v_neck_2 := (SELECT id FROM bien_the_san_pham WHERE sku = 'DEMO-NECK-002-42');
SET @v_bracelet := (SELECT id FROM bien_the_san_pham WHERE sku = 'DEMO-BRACELET-001-16');
SET @v_earring := (SELECT id FROM bien_the_san_pham WHERE sku = 'DEMO-EARRING-001');
...
SELECT @v_ring_1, @demo_staff_id, 'NHAP', 24, 0, 24, 0, 'DEMO: tạo tồn kho ban đầu'
WHERE NOT EXISTS (SELECT 1 FROM lich_su_kho WHERE bien_the_id = @v_ring_1 AND ly_do = 'DEMO: tạo tồn kho ban đầu');

-- Giỏ hàng, yêu thích và mã giảm giá để kiểm tra khu vực khách hàng.
INSERT INTO gio_hang (tai_khoan_id, tong_tien)
VALUES (@demo_customer_id, 4590000.00)
ON DUPLICATE KEY UPDATE tong_tien = VALUES(tong_tien);
SET @demo_cart_id := (SELECT id FROM gio_hang WHERE tai_khoan_id = @demo_customer_id);
INSERT INTO chi_tiet_gio_hang (gio_hang_id, bien_the_id, so_luong, don_gia, thanh_tien, da_chon)
VALUES (@demo_cart_id, @v_neck_1, 1, 4590000.00, 4590000.00, TRUE)
ON DUPLICATE KEY UPDATE so_luong = VALUES(so_luong), don_gia = VALUES(don_gia), thanh_tien = VALUES(thanh_tien), da_chon = TRUE;
INSERT IGNORE INTO yeu_thich (tai_khoan_id, san_pham_id) VALUES (@demo_customer_id, @p_earring);
...
VALUES
  ('DEMO100K', 'Giảm 100.000đ cho đơn từ 3 triệu', 'SO_TIEN', 100000.00, 3000000.00, NULL, 100, 1, '2026-01-01 00:00:00', '2027-12-31 23:59:59', 'HOAT_DONG')
ON DUPLICATE KEY UPDATE
...
SET @demo_coupon_id := (SELECT id FROM ma_giam_gia WHERE ma_code = 'DEMO100K');
...
VALUES
  ('DEMO-ORDER-001', @demo_customer_id, @demo_staff_id, @demo_coupon_id, '2026-08-15 09:30:00', 'HOAN_THANH', 3450000.00, 100000.00, 30000.00, 3380000.00, 'Khách hàng kiểm tra', '0900000001', '12 Lê Lợi, Phường Bến Nghé, Quận 1, TP. Hồ Chí Minh', 'Giao tiêu chuẩn', 'Đơn dữ liệu kiểm tra', 'demo-order-001', REPEAT('a', 64), '2026-08-15 10:00:00', '2026-08-18 14:30:00')
ON DUPLICATE KEY UPDATE
...
SET @demo_order_id := (SELECT id FROM don_hang WHERE ma_don_hang = 'DEMO-ORDER-001');

INSERT INTO chi_tiet_don_hang
  (don_hang_id, bien_the_id, ten_san_pham_chot, sku_chot, size_chot, mau_sac_chot, so_luong, don_gia, thanh_tien)
SELECT @demo_order_id, @v_ring_1, 'Nhẫn vàng NÉT Mảnh', 'DEMO-RING-001-12', '12', 'Vàng', 1, 3450000.00, 3450000.00
WHERE NOT EXISTS (SELECT 1 FROM chi_tiet_don_hang WHERE don_hang_id = @demo_order_id AND bien_the_id = @v_ring_1);
INSERT INTO su_dung_ma_giam_gia (ma_giam_gia_id, tai_khoan_id, don_hang_id, so_tien_giam)
SELECT @demo_coupon_id, @demo_customer_id, @demo_order_id, 100000.00
WHERE NOT EXISTS (SELECT 1 FROM su_dung_ma_giam_gia WHERE don_hang_id = @demo_order_id);
INSERT INTO thanh_toan (don_hang_id, so_tien, phuong_thuc, trang_thai, thoi_gian_thanh_toan)
VALUES (@demo_order_id, 3380000.00, 'COD', 'CONFIRMED', '2026-08-18 14:30:00')
ON DUPLICATE KEY UPDATE so_tien = VALUES(so_tien), trang_thai = 'CONFIRMED', thoi_gian_thanh_toan = VALUES(thoi_gian_thanh_toan);
SET @demo_payment_id := (SELECT id FROM thanh_toan WHERE don_hang_id = @demo_order_id);
INSERT INTO giao_dich (thanh_toan_id, ma_giao_dich_thanh_toan, so_tien, phuong_thuc, trang_thai, thoi_gian, ghi_chu)
SELECT @demo_payment_id, 'DEMO-TXN-001', 3380000.00, 'COD', 'THANH_CONG', '2026-08-18 14:30:00', 'Thu COD cho đơn kiểm tra'
WHERE NOT EXISTS (SELECT 1 FROM giao_dich WHERE ma_giao_dich_thanh_toan = 'DEMO-TXN-001');
...
VALUES (@demo_order_id, 'Giao hàng nội bộ', 'DEMO-SHIP-001', 30000.00, '2026-08-17 18:00:00', '2026-08-16 09:00:00', '2026-08-18 14:30:00', 'DA_GIAO', 'Dữ liệu giao hàng kiểm tra')
...
SELECT @p_ring_1, @demo_customer_id, @demo_order_id, 5, 'Thiết kế gọn, đeo vừa tay và đóng gói cẩn thận.', 'HIEN_THI', 'Cảm ơn bạn đã tin tưởng NÉT Jewelry.'
WHERE NOT EXISTS (SELECT 1 FROM danh_gia WHERE san_pham_id = @p_ring_1 AND tai_khoan_id = @demo_customer_id AND don_hang_id = @demo_order_id);
SET @demo_review_id := (SELECT id FROM danh_gia WHERE san_pham_id = @p_ring_1 AND tai_khoan_id = @demo_customer_id AND don_hang_id = @demo_order_id LIMIT 1);
INSERT INTO hinh_anh_danh_gia (danh_gia_id, duong_dan, thu_tu)
SELECT @demo_review_id, 'https://images.unsplash.com/photo-1605100804763-247f67b3557e?auto=format&fit=crop&w=900&q=80', 1
WHERE NOT EXISTS (SELECT 1 FROM hinh_anh_danh_gia WHERE danh_gia_id = @demo_review_id AND thu_tu = 1);
...
-- SELECT ma_sku, ten_san_pham, gia FROM san_pham WHERE ma_sku LIKE 'DEMO-%';
-- SELECT ten_dang_nhap, email FROM tai_khoan WHERE ten_dang_nhap LIKE 'demo_%';
